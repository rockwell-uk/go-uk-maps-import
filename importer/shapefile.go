...
var (
	dbFieldsMap = make(map[string]fieldName)
)
...
func doImportShapefile(config Config, shapeFile, sfShortName string) (int, map[string]int, time.Duration, error) {
			...
			case ACTION_CHECK:
				if !config.UseFiles || *config.DB.Engine == engine.EngineSQLite {
					if recordsProcessed > 0 && recordsProcessed%chunkSize == 0 {
						runInserts(config.DB.StorageEngine, sfShortName) //nolint:errcheck
					}
				}
				res <- importResult{}
			case ACTION_DONE:
	...
	var magnitude = int(math.Ceil(float64(recordsInFile) / float64(barInterval)))
	...
	if showBar {
		...
		err := job.Start()
		if err != nil {
			return recordsProcessed, sfRowsGenerated, time.Since(importStarted), fmt.Errorf("%v: %v", funcName, err)
		}
...
func logRate(sfShortName string, importStarted time.Time, recordsProcessed int) (float64, string) {
	...
	rate := getRate(duration, recordsProcessed)
