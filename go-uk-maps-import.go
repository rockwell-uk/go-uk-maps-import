...
func main() {
	...
	if err != nil {
		logger.Log(
			logger.LVL_FATAL,
			fmt.Sprintf("%v: Error clearing logs: %v", funcName, err),
		)
		bailOut(1)
	}
	...
	if err != nil {
		logger.Log(
			logger.LVL_FATAL,
			fmt.Sprintf("%v: Error opening log file: %v", funcName, err),
		)
		bailOut(1)
	}
	...
	if download {
		...
		if err != nil {
			logger.Log(
				logger.LVL_FATAL,
				fmt.Sprintf("%v: Error downloading osdata: %v", funcName, err),
			)
			bailOut(1)
		}
	}
	...
	if err != nil {
		logger.Log(
			logger.LVL_FATAL,
			fmt.Sprintf("%v: Error running config check: %v", funcName, err),
		)
		bailOut(1)
	}
	logger.Log(
	...
	if auto {
		...
		if err != nil {
			logger.Log(
				logger.LVL_FATAL,
				fmt.Sprintf("%v: Error running autoconfig: %v", funcName, err),
			)
			bailOut(1)
		}
	}
	...
	case appConfig.ImporterConfig.DB.CountsOnly:
		...
		if err != nil {
			logger.Log(
				logger.LVL_FATAL,
				fmt.Sprintf("%v: Error running import: %v", funcName, err),
			)
			bailOut(1)
		}
...
func startApp(appConfig *autoconfig.AppConfig) {
	...
	if err != nil {
		logger.Log(
			logger.LVL_FATAL,
			fmt.Sprintf("%v: Error starting database engine: %v", funcName, err),
		)
		bailOut(1)
	}
}
...
func stopApp(appConfig *autoconfig.AppConfig) {
	...
	if err != nil {
		logger.Log(
			logger.LVL_FATAL,
			fmt.Sprintf("%v: Error shutting down database engine %v", funcName, err),
		)
		bailOut(1)
	}
