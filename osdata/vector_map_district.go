...
func getDownloadList() ([]VectorMapDistrictTile, error) {
	...
	defer cancel()
	if err != nil {
		return []VectorMapDistrictTile{}, fmt.Errorf("%v: no response from request to %v [%v]", funcName, osDataAPI, err)
	}
	defer resp.Body.Close()

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return []VectorMapDistrictTile{}, fmt.Errorf("%v: unable to read response body [%v]", funcName, err)
	}

	if err := json.Unmarshal(body, &tiles); err != nil {
		return []VectorMapDistrictTile{}, fmt.Errorf("%v: cannot unmarshal JSON [%v]", funcName, err)
	}
